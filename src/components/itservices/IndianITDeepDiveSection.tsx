...
/* ────────────────────────────────────────────────────────────────────────── */
/* Tab 5 — Valuation Lab (interactive DCF + SOTP + Scenarios)                 */
/* ────────────────────────────────────────────────────────────────────────── */

interface ForecastEditable { fy: string; revenueUSDb: number; opMargin: number; fcfUSDb: number; }
interface SotpEditable     { segment: string; revenueUSDb: number; multiple: number; }

function ValuationLab({ activeKey, onSelect }: { activeKey: CompanyKey; onSelect: (k: CompanyKey) => void }) {
  const co = COMPANIES[activeKey];

  // Editable inputs (reset whenever company changes via key prop)
  return (
    <div className="space-y-6">
      <CompanySwitcher activeKey={activeKey} onSelect={onSelect} />
      <ValuationLabBody key={activeKey} co={co} />
    </div>
  );
}

function ValuationLabBody({ co }: { co: CompanyAutopsy }) {
  const [forecast, setForecast] = useState<ForecastEditable[]>(() => co.valuation.forecast.map(r => ({ ...r })));
  const [wacc, setWacc] = useState(co.valuation.waccDefault);
  const [terminalGrowth, setTerminalGrowth] = useState(co.valuation.terminalGrowthDefault);
  const [sotp, setSotp] = useState<SotpEditable[]>(() => co.valuation.sotp.map(s => ({ ...s })));
  const [sotpHorizon, setSotpHorizon] = useState<'now' | 'fy27'>('now');
  const [scenarioWeights, setScenarioWeights] = useState(() => co.valuation.scenarios.map(s => s.probability));
  ...
  const dcf = useMemo(() => {
    ...
    let evDcf = 0;
    const yearly: { fy: string; pv: number }[] = [];
    forecast.forEach((row, i) => {
      const t = i + 1;
      const pv = row.fcfUSDb / Math.pow(1 + r, t);
      evDcf += pv;
      yearly.push({ fy: row.fy, pv });
    });
    const lastFcf = forecast[forecast.length - 1]?.fcfUSDb ?? 0;
    const tv = (lastFcf * (1 + g)) / (r - g);
    const pvTv = tv / Math.pow(1 + r, forecast.length);
    const enterpriseValue = evDcf + pvTv;
    const equityValue = enterpriseValue + co.valuation.netCashUSDb;
    // Convert to per-share. Forecast is in USD; price may be INR — assume implied USDINR ~83 if needed.
    const usdInr = 83;
    const fairUSD = equityValue * 1e9 / (co.valuation.sharesB * 1e9);
    const fairLocal = co.valuation.priceCurrency === 'INR' ? fairUSD * usdInr : fairUSD;
    const upside = ((fairLocal - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) * 100;
    return { yearly, evDcf, tv, pvTv, enterpriseValue, equityValue, fairLocal, upside };
  }, [forecast, wacc, terminalGrowth, co]);

  // ─── SOTP ───
  const activeSotp = sotpHorizon === 'now' ? sotp : co.valuation.sotpFY27.map(s => ({ ...s }));
  const sotpEv = activeSotp.reduce((sum, s) => sum + s.revenueUSDb * s.multiple, 0);
  const sotpEquity = sotpEv + co.valuation.netCashUSDb;
  const sotpFairUSD = sotpEquity * 1e9 / (co.valuation.sharesB * 1e9);
  const sotpFairLocal = co.valuation.priceCurrency === 'INR' ? sotpFairUSD * 83 : sotpFairUSD;
  const sotpUpside = ((sotpFairLocal - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) * 100;

  // ─── Scenario probability-weighted fair value ───
  const totalProb = scenarioWeights.reduce((s, x) => s + x, 0);
  const weightedFair = co.valuation.scenarios.reduce((sum, s, i) => sum + s.fairLocal * (scenarioWeights[i] / totalProb), 0);
  const weightedUpside = ((weightedFair - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) * 100;

  // ─── Sensitivity grid (WACC × Terminal Growth) ───
  const sensitivity = useMemo(() => {
    const waccs = [wacc - 2, wacc - 1, wacc, wacc + 1, wacc + 2];
    const tgs   = [terminalGrowth - 1, terminalGrowth - 0.5, terminalGrowth, terminalGrowth + 0.5, terminalGrowth + 1];
    const rows = waccs.map(w => {
      ...
      return {
        ...
        cells: tgs.map(tg => {
          const g = tg / 100;
          let evDcf = 0;
          forecast.forEach((row, i) => { evDcf += row.fcfUSDb / Math.pow(1 + r, i + 1); });
          const lastFcf = forecast[forecast.length - 1]?.fcfUSDb ?? 0;
          const tv = (lastFcf * (1 + g)) / (r - g);
          const pvTv = tv / Math.pow(1 + r, forecast.length);
          const ev = evDcf + pvTv;
          const equity = ev + co.valuation.netCashUSDb;
          const fairUSD = equity / co.valuation.sharesB;
          const fairLocal = co.valuation.priceCurrency === 'INR' ? fairUSD * 83 : fairUSD;
          const up = ((fairLocal - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) * 100;
          return { tg, fairLocal, up };
        }),
      };
    });
    return { waccs, tgs, rows };
  }, [forecast, wacc, terminalGrowth, co]);

  const updateForecast = (i: number, field: keyof ForecastEditable, value: number) => {
    setForecast(prev => {
      const next = prev.map(r => ({ ...r }));
      (next[i] as any)[field] = value;
      // Recompute FCF if revenue or margin changed (assume FCF/Rev = OpMargin × 0.78 (after-tax) × 0.85 (FCF conversion))
      if (field === 'revenueUSDb' || field === 'opMargin') {
        next[i].fcfUSDb = +(next[i].revenueUSDb * (next[i].opMargin / 100) * 0.78 * 0.85).toFixed(2);
      }
      return next;
    });
  };
  ...
  const resetForecast = () => {
    setForecast(co.valuation.forecast.map(r => ({ ...r })));
    setWacc(co.valuation.waccDefault);
    setTerminalGrowth(co.valuation.terminalGrowthDefault);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiBlock label="Current Price"          value={fmtCurr(co.valuation.currentPriceLocal, co.valuation.priceCurrency)} subtle="market quote" tone="blue" />
        <KpiBlock label="DCF Fair Value"         value={fmtCurr(dcf.fairLocal, co.valuation.priceCurrency)} subtle={`${dcf.upside >= 0 ? '+' : ''}${dcf.upside.toFixed(1)}% upside`} tone={dcf.upside >= 0 ? 'green' : 'red'} />
        <KpiBlock label="SOTP Fair Value"        value={fmtCurr(sotpFairLocal, co.valuation.priceCurrency)} subtle={`${sotpUpside >= 0 ? '+' : ''}${sotpUpside.toFixed(1)}% upside`}    tone={sotpUpside >= 0 ? 'green' : 'red'} />
        <KpiBlock label="Probability-Weighted"   value={fmtCurr(weightedFair, co.valuation.priceCurrency)} subtle={`${weightedUpside >= 0 ? '+' : ''}${weightedUpside.toFixed(1)}% upside`} tone={weightedUpside >= 0 ? 'green' : 'red'} />
      </div>

      {/* DCF block */}
      <SectionCard
        title="Discounted Cash Flow — Edit Forecast Live"
        icon={<Gauge size={14} />}
        action={
          <button onClick={resetForecast} className="text-[11px] text-amber-300 hover:text-amber-100 uppercase tracking-widest">
            Reset
          </button>
        }
      >
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
          <RangeSlider label="WACC" value={wacc} min={7} max={15} step={0.25} onChange={setWacc} suffix="%" />
          <RangeSlider label="Terminal Growth (g)" value={terminalGrowth} min={1} max={6} step={0.25} onChange={setTerminalGrowth} suffix="%" />
          <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
            <div className="text-[10px] uppercase tracking-widest text-slate-500">Implied Spread (r − g)</div>
            <div className="font-mono text-xl text-amber-200 mt-1">{(wacc - terminalGrowth).toFixed(2)}%</div>
            <div className="text-[11px] text-slate-400 mt-1">Below ~5% the terminal value dominates and the model becomes unstable.</div>
          </div>
        </div>
          ...
          <table className="w-full text-sm sensex-table">
            <thead>
              <tr>
                ...
                <th className="text-right px-2 py-1.5">Revenue ($B)</th>
                <th className="text-right px-2 py-1.5">Op Margin %</th>
                <th className="text-right px-2 py-1.5">FCF ($B)</th>
                <th className="text-right px-2 py-1.5">PV ($B)</th>
              </tr>
            </thead>
            <tbody>
              {forecast.map((row, i) => (
                <tr key={row.fy} className="border-t border-slate-800/60 hover:bg-slate-800/30">
                  <td className="px-2 py-1.5 font-semibold text-amber-100">{row.fy}</td>
                  <td className="px-2 py-1.5 text-right">
                    <NumberInput value={row.revenueUSDb} step={0.5} min={0} onChange={v => updateForecast(i, 'revenueUSDb', v)} />
                  </td>
                  <td className="px-2 py-1.5 text-right">
                    <NumberInput value={row.opMargin} step={0.5} min={0} max={50} onChange={v => updateForecast(i, 'opMargin', v)} />
                  </td>
                  <td className="px-2 py-1.5 text-right">
                    <NumberInput value={row.fcfUSDb} step={0.1} min={0} onChange={v => updateForecast(i, 'fcfUSDb', v)} />
                  </td>
                  <td className="px-2 py-1.5 text-right text-amber-200 font-mono">{dcf.yearly[i] ? dcf.yearly[i].pv.toFixed(2) : '—'}</td>
                </tr>
              ))}
              <tr className="border-t border-amber-500/40 bg-amber-500/5">
                <td className="px-2 py-1.5 font-semibold text-amber-200">Terminal Value (PV)</td>
                <td colSpan={3} className="px-2 py-1.5 text-right text-slate-400">@ g={terminalGrowth.toFixed(2)}% / r={wacc.toFixed(2)}%</td>
                <td className="px-2 py-1.5 text-right text-amber-200 font-mono">{dcf.pvTv.toFixed(2)}</td>
              </tr>
              <tr className="border-t border-slate-700/60 bg-slate-900/60">
                <td className="px-2 py-1.5 font-semibold text-amber-100">Enterprise Value ($B)</td>
                <td colSpan={3} className="px-2 py-1.5 text-right text-slate-400">+ Net Cash ${co.valuation.netCashUSDb.toFixed(2)}B</td>
                <td className="px-2 py-1.5 text-right text-amber-100 font-mono font-semibold">{dcf.enterpriseValue.toFixed(2)}</td>
              </tr>
              <tr className="border-t border-slate-700/60 bg-slate-900/60">
                <td className="px-2 py-1.5 font-semibold text-amber-100">Equity Value ($B) / Shares (B)</td>
                <td colSpan={3} className="px-2 py-1.5 text-right text-slate-400">{dcf.equityValue.toFixed(2)} / {co.valuation.sharesB.toFixed(2)}</td>
                <td className="px-2 py-1.5 text-right text-amber-100 font-mono font-semibold">{fmtCurr(dcf.fairLocal, co.valuation.priceCurrency)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="h-[220px] mt-4">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={forecast} margin={{ top: 8, right: 16, left: 0, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              <XAxis dataKey="fy" tick={{ fill: PALETTE.muted, fontSize: 10 }} />
              <YAxis yAxisId="L" tick={{ fill: PALETTE.muted, fontSize: 10 }} label={{ value: 'Revenue $B', angle: -90, position: 'insideLeft', fill: PALETTE.muted, fontSize: 10 }} />
              <YAxis yAxisId="R" orientation="right" tick={{ fill: PALETTE.muted, fontSize: 10 }} domain={[15, 30]} />
              <Tooltip content={<ChartTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11, color: PALETTE.muted }} />
              <Bar yAxisId="L" dataKey="revenueUSDb" fill={PALETTE.gold} name="Revenue ($B)" radius={[3,3,0,0]} />
              <Line yAxisId="L" type="monotone" dataKey="fcfUSDb"   stroke={PALETTE.green} strokeWidth={2} name="FCF ($B)" dot={{ r: 3 }} />
              <Line yAxisId="R" type="monotone" dataKey="opMargin" stroke={PALETTE.blue}  strokeWidth={2} name="Op Margin %" dot={{ r: 3 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </SectionCard>
      ...
      {/* Sensitivity */}
      <SectionCard title="DCF Sensitivity — WACC × Terminal Growth" icon={<Activity size={14} />}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr>
                <th className="text-left px-2 py-1.5 text-[10px] uppercase tracking-widest text-slate-500">WACC ↓ / g →</th>
                {sensitivity.tgs.map(g => (
                  <th key={g} className="text-right px-2 py-1.5 text-[10px] uppercase tracking-widest text-slate-500">{g.toFixed(2)}%</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sensitivity.rows.map(row => (
                <tr key={row.wacc} className="border-t border-slate-800/60">
                  <td className="px-2 py-1.5 font-semibold text-amber-100">{row.wacc.toFixed(2)}%</td>
                  {row.cells.map((c, i) => {
                    // heatmap: green if upside, red if downside
                    const intensity = Math.min(1, Math.abs(c.up) / 60);
                    const color = c.up >= 0
                      ? `rgba(16, 185, 129, ${0.10 + 0.35 * intensity})`
                      : `rgba(239, 68, 68, ${0.10 + 0.35 * intensity})`;
                    const isCenter = row.wacc === wacc && c.tg === terminalGrowth;
                    return (
                      <td key={i} className="px-2 py-2 text-right font-mono text-xs"
                          style={{ background: color, outline: isCenter ? `2px solid ${PALETTE.gold}` : undefined, outlineOffset: '-2px' }}>
                        <div className="text-slate-100">{fmtCurr(c.fairLocal, co.valuation.priceCurrency)}</div>
                        <div className={c.up >= 0 ? 'text-emerald-200' : 'text-red-200'}>{c.up >= 0 ? '+' : ''}{c.up.toFixed(0)}%</div>
                      </td>
                    );
                  })}
                </tr>
        ...
        <p className="text-[11px] text-slate-500 mt-2">
          The highlighted cell reflects the live inputs above. Each cell shows the implied fair value per share and the upside vs current price.
        </p>
      </SectionCard>

      {/* SOTP */}
      <SectionCard
        title="Sum-of-Parts (SOTP) Builder"
        icon={<Layers size={14} />}
        action={
          <div className="segmented">
            <button className={sotpHorizon === 'now' ? 'active' : ''} onClick={() => { setSotpHorizon('now'); setSotp(co.valuation.sotp.map(s => ({ ...s }))); }}>FY24 Mix</button>
            <button className={sotpHorizon === 'fy27' ? 'active' : ''} onClick={() => { setSotpHorizon('fy27'); setSotp(co.valuation.sotpFY27.map(s => ({ ...s }))); }}>FY27 Bull Mix</button>
          </div>
        }
      >
        <div className="overflow-x-auto">
          <table className="w-full text-sm sensex-table">
            <thead>
              <tr>
                ...
                <th className="text-right px-2 py-1.5">Revenue ($B)</th>
                <th className="text-right px-2 py-1.5">Multiple (x rev)</th>
                <th className="text-right px-2 py-1.5">EV ($B)</th>
              </tr>
            </thead>
            <tbody>
              {(sotpHorizon === 'now' ? sotp : co.valuation.sotpFY27).map((s, i) => {
                const onChange = (field: 'revenueUSDb' | 'multiple', v: number) => {
                  if (sotpHorizon !== 'now') return; // only edit FY24 baseline
                  setSotp(prev => {
                    const next = prev.map(r => ({ ...r }));
                ...
                return (
                  <tr key={s.segment} className="border-t border-slate-800/60 hover:bg-slate-800/30">
                    <td className="px-2 py-1.5 text-amber-100">{s.segment}</td>
                    <td className="px-2 py-1.5 text-right">
                      {sotpHorizon === 'now'
                        ? <NumberInput value={s.revenueUSDb} step={0.1} min={0} onChange={v => onChange('revenueUSDb', v)} />
                        : <span className="font-mono text-slate-200">{s.revenueUSDb.toFixed(2)}</span>}
                    </td>
                    <td className="px-2 py-1.5 text-right">
                      {sotpHorizon === 'now'
                        ? <NumberInput value={s.multiple} step={0.1} min={0} onChange={v => onChange('multiple', v)} />
                        : <span className="font-mono text-slate-200">{s.multiple.toFixed(2)}</span>}
                    </td>
                    <td className="px-2 py-1.5 text-right text-amber-200 font-mono">{(s.revenueUSDb * s.multiple).toFixed(2)}</td>
                  </tr>
                );
              })}
              <tr className="border-t border-amber-500/40 bg-amber-500/5">
                <td className="px-2 py-1.5 font-semibold text-amber-100">Enterprise Value</td>
                <td colSpan={2} className="px-2 py-1.5 text-right text-slate-400">aggregate of segment values</td>
                <td className="px-2 py-1.5 text-right text-amber-100 font-mono font-semibold">{sotpEv.toFixed(2)}</td>
              </tr>
              <tr className="border-t border-slate-700/60 bg-slate-900/60">
                <td className="px-2 py-1.5 font-semibold text-amber-100">Equity Value / Fair Value per Share</td>
                <td colSpan={2} className="px-2 py-1.5 text-right text-slate-400">+ Net Cash ${co.valuation.netCashUSDb.toFixed(2)}B / {co.valuation.sharesB.toFixed(2)}B shares</td>
                <td className="px-2 py-1.5 text-right text-amber-100 font-mono font-semibold">{fmtCurr(sotpFairLocal, co.valuation.priceCurrency)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {/* Scenario probability mixer */}
      <SectionCard title="Scenario Probability Mixer" icon={<Briefcase size={14} />}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {co.valuation.scenarios.map((s, i) => {
            const colors: Record<string, string> = { Bull: 'border-emerald-500/40', Base: 'border-amber-500/40', Bear: 'border-red-500/40' };
            return (
              <div key={s.name} className={`rounded-lg border ${colors[s.name]} bg-slate-900/40 p-3`}>
                <div className="flex items-center justify-between gap-2 mb-1">
                  <div className="text-amber-100 font-semibold">{s.name} Case</div>
                  <span className="font-mono text-xs text-slate-300">{(scenarioWeights[i] / totalProb * 100).toFixed(0)}%</span>
                </div>
                <div className="text-2xl font-serif text-amber-200 tracking-tight">{fmtCurr(s.fairLocal, co.valuation.priceCurrency)}</div>
                <div className={`text-xs font-semibold ${((s.fairLocal - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>
                  {(((s.fairLocal - co.valuation.currentPriceLocal) / co.valuation.currentPriceLocal) * 100).toFixed(1)}% upside
                </div>
                <input
                  type="range"
                  className="range-slider w-full mt-3"
                  min={0} max={100} step={1}
                  value={scenarioWeights[i]}
                  onChange={e => {
                    const v = +e.target.value;
                    setScenarioWeights(prev => prev.map((p, idx) => idx === i ? v : p));
                  }}
                />
                <div className="text-[11px] text-slate-400 mt-2 leading-relaxed">{s.commentary}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
            <div className="text-[10px] uppercase tracking-widest text-slate-500">Probability-Weighted Fair Value</div>
            <div className="text-2xl font-serif text-amber-200 tracking-tight mt-1">{fmtCurr(weightedFair, co.valuation.priceCurrency)}</div>
            <div className={`text-xs font-semibold ${weightedUpside >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>{weightedUpside >= 0 ? '+' : ''}{weightedUpside.toFixed(1)}% upside</div>
          </div>
          <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
            <div className="text-[10px] uppercase tracking-widest text-slate-500">Total Probability</div>
            <div className="text-2xl font-serif text-amber-200 tracking-tight mt-1">{totalProb.toFixed(0)}%</div>
            <div className="text-xs text-slate-400">Slide weights up/down — model auto-normalises.</div>
          </div>
          <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
            <div className="text-[10px] uppercase tracking-widest text-slate-500">Final Recommendation</div>
            <div className="text-2xl font-serif tracking-tight mt-1" style={{ color: RATING_COLOR[co.rating] }}>{co.rating}</div>
            <div className="text-xs text-slate-400">{co.thesisLine}</div>
          </div>
        </div>
      </SectionCard>

      {/* Relative comparison */}
      <SectionCard title="Relative Multiples vs Peers" icon={<LineIcon size={14} />}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm sensex-table">
            <thead>
              <tr>
                ...
                <th className="text-right px-2 py-1.5">P/E</th>
                <th className="text-right px-2 py-1.5">EV / EBITDA</th>
                <th className="text-right px-2 py-1.5">P / FCF</th>
              </tr>
            </thead>
            <tbody>
              {co.valuation.relativeMultiples.map(p => (
                <tr key={p.peer} className={`border-t border-slate-800/60 ${p.peer === co.name ? 'bg-amber-500/5' : 'hover:bg-slate-800/30'}`}>
                  <td className="px-2 py-1.5 text-amber-100 font-semibold">{p.peer}{p.peer === co.name && <span className="text-[10px] text-amber-400 ml-2">← active</span>}</td>
                  <td className="px-2 py-1.5 text-right">{p.pe}x</td>
                  <td className="px-2 py-1.5 text-right">{p.evEbitda}x</td>
                  <td className="px-2 py-1.5 text-right">{p.pFcf}x</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function RangeSlider({ label, value, min, max, step, onChange, suffix }: { label: string; value: number; min: number; max: number; step: number; onChange: (v: number) => void; suffix?: string }) {
  return (
    <div className="rounded-lg border border-slate-700/60 bg-slate-900/40 p-3">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">{label}</span>
        <span className="font-mono text-amber-200 text-sm">{value.toFixed(2)}{suffix}</span>
      </div>
      <input
        type="range"
...
function NumberInput({ value, step, min, max, onChange }: { value: number; step: number; min?: number; max?: number; onChange: (v: number) => void }) {
  return (
    <input
      ...
      max={max}
      onChange={e => onChange(+e.target.value)}
      className="w-20 bg-slate-900/60 border border-slate-700/60 rounded px-2 py-1 text-right font-mono text-amber-200 text-xs focus:outline-none focus:border-amber-500/50"
    />
  );
}
